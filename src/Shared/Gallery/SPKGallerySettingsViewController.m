...
- (void)rebuildSections {
    ...
    for (NSNumber *sourceValue in sources) {
        ...
        SPKSetting *row = [SPKSetting switchCellWithTitle:[SPKGalleryFile labelForSource:source]
                                              ...
                                              defaultsKey:@""];
        row.switchValueProvider = ^BOOL{ return SPKGallerySourceIsHidden(source); };
        row.switchChangeHandler = ^(BOOL hidden) { SPKGallerySetSourceHidden(source, hidden); };
        [rows addObject:row];
    }
...
+ (NSArray *)searchSections {
    return @[
        SPKTopicSection(@"Storage", @[
            [SPKSetting valueCellWithTitle:@"Total" subtitle:@"Gallery storage and file count" icon:SPKSettingsIcon(@"info")],
            [SPKSetting valueCellWithTitle:@"Images" subtitle:@"Saved image count" icon:SPKSettingsIcon(@"photo")],
            [SPKSetting valueCellWithTitle:@"Videos" subtitle:@"Saved video count" icon:SPKSettingsIcon(@"video")],
            [SPKSetting valueCellWithTitle:@"Audio" subtitle:@"Saved audio count" icon:SPKSettingsIcon(@"audio")]
        ], nil),
        SPKTopicSection(@"Browsing", @[
            [SPKSetting switchCellWithTitle:@"Show Favorites at Top" icon:SPKSettingsIcon(@"heart") defaultsKey:kFavoritesAtTopKey],
            [SPKSetting navigationCellWithTitle:@"Hidden Sources" subtitle:@"" icon:SPKSettingsIcon(@"eye_off") viewController:[SPKGalleryHiddenSourcesViewController new]]
        ], @"Pin favorites above other files inside the current sort and folder context."),
        SPKTopicSection(@"Lock", @[
            [SPKSetting switchCellWithTitle:@"Gallery Passcode Lock" icon:SPKSettingsIcon(@"lock") defaultsKey:@""],
            [SPKSetting buttonCellWithTitle:@"Change Passcode" subtitle:nil icon:SPKSettingsIcon(@"key") action:^{}]
        ...
        SPKTopicSection(@"Import", @[
            [SPKSetting buttonCellWithTitle:@"Import from Files..." subtitle:nil icon:SPKSettingsIcon(@"arrow_down") action:^{}]
        ], @"Import from the Files app with full editable metadata."),
        SPKTopicSection(@"Delete", @[
            [SPKSetting buttonCellWithTitle:@"Delete Files" subtitle:nil icon:SPKSettingsIcon(@"trash") action:^{}]
        ], nil)
    ];
}
...
- (void)rebuildSections {
    ...
    [sections addObject:SPKTopicSection(@"Storage", @[
        [SPKSetting valueCellWithTitle:@"Total" subtitle:[NSString stringWithFormat:@"%ld files • %@", (long)self.stats.totalFiles, [self formattedSize:self.stats.totalSize]] icon:SPKSettingsIcon(@"info")],
        [SPKSetting valueCellWithTitle:@"Images" subtitle:[NSString stringWithFormat:@"%ld", (long)self.stats.imageCount] icon:SPKSettingsIcon(@"photo")],
        [SPKSetting valueCellWithTitle:@"Videos" subtitle:[NSString stringWithFormat:@"%ld", (long)self.stats.videoCount] icon:SPKSettingsIcon(@"video")],
        [SPKSetting valueCellWithTitle:@"Audio" subtitle:[NSString stringWithFormat:@"%ld", (long)self.stats.audioCount] icon:SPKSettingsIcon(@"audio")]
    ], nil)];
    ...
    if (mgr.isLockEnabled) {
        SPKSetting *changePasscode = [SPKSetting buttonCellWithTitle:@"Change Passcode" subtitle:nil icon:SPKSettingsIcon(@"key") action:^{
    ...
    SPKSetting *importRow = [SPKSetting buttonCellWithTitle:@"Import from Files..." subtitle:nil icon:SPKSettingsIcon(@"arrow_down") action:^{
    ...
    SPKSetting *deleteRow = [SPKSetting buttonCellWithTitle:@"Delete Files" subtitle:nil icon:SPKSettingsIcon(@"trash") action:^{
...
- (void)promptClaimUnassignedFiles {
    ...
    [SPKIGAlertPresenter presentAlertFromViewController:self
                                               ...
                                               message:message
                                               actions:@[
        [SPKIGAlertAction actionWithTitle:@"Not Now" style:SPKIGAlertActionStyleCancel handler:nil],
        [SPKIGAlertAction actionWithTitle:@"Assign" style:SPKIGAlertActionStyleDefault handler:^{
