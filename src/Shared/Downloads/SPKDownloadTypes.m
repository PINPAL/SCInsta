...
BOOL SPKDownloadStateAllowsTransition(SPKDownloadState from, SPKDownloadState to) {
    ...
    if (SPKDownloadStateIsTerminal(from)) return NO;
    switch (from) {
...
SPKDownloadState SPKDownloadDerivedJobState(NSArray<NSNumber *> *itemStates) {
    if (itemStates.count == 0) return SPKDownloadStatePending;
    BOOL anyRunning = NO;
    ...
    for (NSNumber *n in itemStates) {
        ...
        if (s == SPKDownloadStateRunning) anyRunning = YES;
        if (s == SPKDownloadStateFinalizing) anyFinalizing = YES;
        if (s == SPKDownloadStatePending || s == SPKDownloadStateWaitingForPreflight || s == SPKDownloadStateQueued) anyQueuedLike = YES;
        if (s == SPKDownloadStateSucceeded) succeeded++;
        else if (s == SPKDownloadStateFailed) failed++;
        else if (s == SPKDownloadStateCancelled) cancelled++;
        else if (s == SPKDownloadStateInterrupted) interrupted++;
    }
    if (anyRunning || anyFinalizing) return SPKDownloadStateRunning;
    if (anyQueuedLike) return SPKDownloadStateQueued;
    NSUInteger total = itemStates.count;
    if (succeeded == total) return SPKDownloadStateSucceeded;
    if (failed == total) return SPKDownloadStateFailed;
    if (cancelled == total) return SPKDownloadStateCancelled;
    if (interrupted == total) return SPKDownloadStateInterrupted;
    if (succeeded > 0 && (failed + cancelled + interrupted) > 0) return SPKDownloadStatePartial;
    if (failed > 0 && succeeded == 0 && cancelled == 0 && interrupted == 0) return SPKDownloadStateFailed;
    if (cancelled > 0 && succeeded == 0) return SPKDownloadStateCancelled;
    if (interrupted > 0 && succeeded == 0) return SPKDownloadStateInterrupted;
...
NSString *SPKDownloadDestinationDisplayName(SPKDownloadDestination destination) {
    switch (destination) {
        ...
        case SPKDownloadDestinationGallery: return @"Gallery";
        case SPKDownloadDestinationShare: return @"Share";
        case SPKDownloadDestinationClipboard: return @"Clipboard";
        case SPKDownloadDestinationCacheOnly: return @"Download";
    }
    return @"Download";
}
...
NSString *SPKDownloadSourceSurfaceDisplayName(SPKDownloadSourceSurface surface) {
    switch (surface) {
        case SPKDownloadSourceSurfaceFeed: return @"Feed";
        case SPKDownloadSourceSurfaceReels: return @"Reels";
        case SPKDownloadSourceSurfaceStories: return @"Stories";
        case SPKDownloadSourceSurfaceDirect: return @"Direct";
        case SPKDownloadSourceSurfaceAudioPage: return @"Audio";
        case SPKDownloadSourceSurfaceMediaPreview: return @"Preview";
        case SPKDownloadSourceSurfaceGallery: return @"Gallery";
        case SPKDownloadSourceSurfaceProfile: return @"Profile";
