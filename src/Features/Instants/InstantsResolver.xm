...
- (NSURL *)url { return self.sparkleMediaURL; }
- (NSURL *)imageURL { return self.sparklePhotoURL ?: self.sparkleMediaURL; }
- (NSURL *)videoURL { return self.sparkleVideoURL; }
...
static id SPKInstantsSwiftIvarValue(id target, NSString *key) {
    ...
    for (NSString *candidateKey in @[key, [@"_" stringByAppendingString:key]]) {
        id value = nil;
        @try { value = [SPKUtils getIvarForObj:target name:candidateKey.UTF8String]; }
...
static id SPKInstantsObjectValue(id target, NSArray<NSString *> *keys) {
    ...
    for (NSString *key in keys) {
        ...
        if (!value) value = SPKInstantsSwiftIvarValue(target, key);
        if (!value) value = SPKInstantsObjectIvarValue(target, key);
        if (value && ![value isKindOfClass:NSNull.class]) return value;
    }
    return nil;
}
...
static BOOL SPKInstantsObjectLooksVideo(id object) {
    ...
    if (SPKInstantsDoubleValue(object, @[@"videoDuration", @"duration"]) > 0.0) return YES;
    if (SPKInstantsObjectValue(object, @[@"videoVersions", @"video_versions", @"videoVersionsFragment"])) return YES;
    ...
    if (videoFragment && videoFragment != object) {
        if (SPKInstantsVideoURLForObject(videoFragment)) return YES;
        if (SPKInstantsDoubleValue(videoFragment, @[@"videoDuration", @"duration"]) > 0.0) return YES;
        if (SPKInstantsObjectValue(videoFragment, @[@"videoVersions", @"video_versions", @"videoVersionsFragment"])) return YES;
    }
    return NO;
}
...
static NSURL *SPKInstantsVideoURLForObject(id object) {
    ...
    if (SPKInstantsURLLooksVideo(directURL)) return directURL;
    ...
    if (nested && nested != object) return SPKInstantsVideoURLForObject(nested);
...
static NSURL *SPKInstantsPhotoURLForObject(id object) {
    ...
    NSURL *directURL = SPKURLFromValue(object);
    if (directURL && !SPKInstantsURLLooksVideo(directURL)) return directURL;
    ...
    if (url && !SPKInstantsURLLooksVideo(url)) return url;
    ...
    id nested = SPKInstantsObjectValue(object, SPKInstantsNestedKeys());
    if (nested && nested != object) return SPKInstantsPhotoURLForObject(nested);
...
static NSString *SPKInstantsMediaPKForObject(id object, NSInteger depth) {
    ...
    id nested = SPKInstantsObjectValue(object, SPKInstantsNestedKeys());
    if (nested && nested != object) return SPKInstantsMediaPKForObject(nested, depth + 1);
    return nil;
}
...
static NSDate *SPKInstantsPostedDateForObject(id object, NSInteger depth) {
    ...
    id nested = SPKInstantsObjectValue(object, SPKInstantsNestedKeys());
    if (nested && nested != object) return SPKInstantsPostedDateForObject(nested, depth + 1);
    return nil;
}
...
static void SPKInstantsInitTracking(UIView *stackView) {
    ...
    @try { currentImages = SPKArrayFromCollection([stackView valueForKey:@"currentImages"]); } @catch (__unused NSException *e) {}
...
static NSInteger SPKInstantsVisualActiveIndex(UIView *stackView, NSArray *currentImages) {
    ...
    if (backingMedia) activePK = SPKInstantsMediaPKForObject(backingMedia, 0);
...
static id SPKInstantsBackingObjectFromView(UIView *view, NSInteger depth) {
    ...
    for (NSString *key in @[@"media", @"item", @"model", @"viewModel", @"legacyViewModel", @"quickSnapInfo", @"snap", @"instantSnap"]) {
        ...
        if (!value) value = SPKKVCObject(view, key);
...
static SPKInstantsResolvedSnap *SPKInstantsResolvedSnapFromView(UIView *snap) {
    ...
    if (!backingMedia) backingMedia = SPKInstantsBackingObjectFromView(imageView, 0);
    ...
    if (!videoURL) videoURL = SPKInstantsVideoURLFromSnapPlayback(snap);
    if (!photoURL) photoURL = SPKInstantsURLForImageView(imageView);
    if (!photoURL && imageView.image) photoURL = SPKInstantsTempURLForImage(imageView.image);

    BOOL looksVideo = NO;
    if (backingMedia) looksVideo = SPKInstantsObjectLooksVideo(backingMedia);
    if (!looksVideo) looksVideo = (SPKInstantsVideoViewInSnap(snap) != nil);
    BOOL isVideo = looksVideo && videoURL != nil;
...
static NSString *SPKInstantsCurrentAuthorUsername(UIView *header) {
    ...
    while (idx < queue.count) {
        ...
        if ([className containsString:@"IGQuickSnapConsumptionAuthorTextView"]) {
            ...
            if (!label) label = [SPKUtils getIvarForObj:view name:"usernameLabel"];
...
static NSArray *SPKInstantsStoreSnapshot(void) {
    ...
    if (!store) store = SPKInstantsSwiftIvarValue(service, @"quickSnapStore");
    if (!store) {
    ...
    if (!timeOrdered) timeOrdered = SPKInstantsSwiftIvarValue(store, @"timeOrderedQuicksnaps");
...
SPKInstantsResolverResult *SPKInstantsResolveForHeader(UIView *header, NSString *reason) {
    ...
    if (stackView && !sTrackedIndexValid) SPKInstantsInitTracking(stackView);
