...
- (BOOL)isEmpty {
    return self.searchText.length == 0
        && !self.hasKindFilter
        && self.dateRange == SPKDMDateRangeAll;
}
...
- (NSDate *)effectiveStart {
    if (self.dateRange == SPKDMDateRangeCustom) return self.customStart;
    NSCalendar *cal = NSCalendar.currentCalendar;
    NSDate *now = [NSDate date];
    switch (self.dateRange) {
        case SPKDMDateRangeToday: return [cal startOfDayForDate:now];
        case SPKDMDateRangeWeek:  return [cal dateByAddingUnit:NSCalendarUnitDay  value:-7  toDate:now options:0];
        case SPKDMDateRangeMonth: return [cal dateByAddingUnit:NSCalendarUnitDay  value:-30 toDate:now options:0];
        default:                  return nil;
    }
}

- (NSDate *)effectiveEnd {
    if (self.dateRange == SPKDMDateRangeCustom) return self.customEnd;
    return nil;
}
...
- (BOOL)matchDateForMessage:(SPKDeletedMessage *)m {
    ...
    if (!key) return self.dateRange == SPKDMDateRangeAll;
