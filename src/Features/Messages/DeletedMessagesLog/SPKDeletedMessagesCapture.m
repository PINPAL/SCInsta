...
static NSString *spkTryStringSelectors(id obj, NSArray<NSString *> *names) {
    ...
    for (NSString *n in names) {
        ...
        @try {
            ...
            if (!str.length || spkIsDescriptionFallback(str)) continue;
            return str;
        } @catch (__unused id e) {}
    }
    return nil;
...
static void spkScanForURLsRecursive(id obj, int depth,
    ...
    if ([obj isKindOfClass:[NSURL class]]) {
        ...
        if (s.length) spkScanForURLsRecursive(s, depth, outMedia, mediaScore, outThumb, thumbScore, parentName);
        return;
    }
    if ([obj isKindOfClass:[NSArray class]]) {
        for (id e in (NSArray *)obj) spkScanForURLsRecursive(e, depth - 1, outMedia, mediaScore, outThumb, thumbScore, parentName);
        return;
    }
    if ([obj isKindOfClass:[NSDictionary class]]) {
        NSDictionary *d = obj;
...
static void spkCollectIvarNames(id obj, int depth, NSMutableSet *visited, NSMutableSet<NSString *> *out) {
    if (!obj || depth < 0) return;
    if ([obj isKindOfClass:[NSArray class]]) {
        for (id e in (NSArray *)obj) spkCollectIvarNames(e, depth - 1, visited, out);
        return;
    }
    if ([obj isKindOfClass:[NSDictionary class]]) {
        NSDictionary *d = obj;
...
static NSString *spkSidFromMessage(id m) {
    ...
    if (!sid.length) {
        ...
        if (key) sid = spkStrIvar(key, "_serverId") ?: spkStrIvar(key, "_messageServerId");
    }
    return sid;
}
...
static void spkScanVoiceMetadata(id media, double *outDuration, NSArray **outWaveform) {
    ...
    while (stack.count) {
        ...
        if (!*outDuration) {
            ...
            if (cand <= 0) cand = spkDoubleSelector(cur, @"duration");
            if (cand <= 0) {
                for (Class c = [cur class]; c && c != [NSObject class]; c = class_getSuperclass(c)) {
...
static void spkCollectImageURL(id obj, int depth, NSMutableSet *visited, NSString **out) {
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        if (spkURLLooksLikeImage(obj)) *out = obj;
        return;
    }
    if ([obj isKindOfClass:[NSURL class]]) {
    ...
    if ([obj isKindOfClass:[NSArray class]] || [obj isKindOfClass:[NSSet class]]
        ...
        for (id e in obj) { spkCollectImageURL(e, depth - 1, visited, out); if (*out) return; }
        return;
    }
    if ([obj isKindOfClass:[NSDictionary class]]) {
...
static NSDictionary *spkBuildSnapshot(id message, NSString *ownerHint) {
    ...
    if (spkAnyIvar(content, "_threadActivity")
        || spkAnyIvar(content, "_messageTypeNotLocallyAvailable_placeholderTitle")
        || spkAnyIvar(content, "_messageTypeNotLocallyAvailable_placeholderMessage")
        || spkAnyIvar(content, "_expiredPlaceholder_messageContent")) {
        return nil;
    }
    ...
    if (media) {
        ...
        if (!hasSemanticSticker) semanticSticker = spkBoolIvar(media, "_animatedMedia_isSticker", &hasSemanticSticker);

        NSMutableSet *vis = [NSMutableSet set];
        NSMutableSet<NSString *> *tokens = [NSMutableSet set];
        spkCollectIvarNames(media, 5, vis, tokens);

        if (stickerPayload || (hasSemanticSticker && semanticSticker)) kind = SPKDeletedMessageKindSticker;
        else if (instamadilloGif || giphy)                         kind = SPKDeletedMessageKindGif;
        else if (spkSetContainsAny(tokens, @[@"voice", @"audio"])) kind = SPKDeletedMessageKindVoice;
        else if (spkSetContainsAny(tokens, @[@"sticker"]))         kind = SPKDeletedMessageKindSticker;
        else if (spkSetContainsAny(tokens, @[@"giphy", @"gif", @"animated"])) kind = SPKDeletedMessageKindGif;
        else if (spkSetContainsAny(tokens, @[@"video", @"dashmanifest", @"playableurl"])) kind = SPKDeletedMessageKindVideo;
        else                                                       kind = SPKDeletedMessageKindPhoto;
        ...
        if (kind == SPKDeletedMessageKindGif || kind == SPKDeletedMessageKindSticker) {
            ...
            if (!explicitURL.length) explicitURL = spkGiphyMediaURL(giphy);
            if (!explicitURL.length && stickerPayload) explicitURL = spkStickerMediaURL(stickerPayload);
        ...
        if (kind == SPKDeletedMessageKindVideo) {
            ...
            if (!video) {
                if (visualPayload) {
                    ...
                    if (!overlayPhoto) overlayPhoto = spkAnyIvar(visualPayload, "_video_overlayPhoto")
    ...
    if (kind == SPKDeletedMessageKindUnknown) {
        ...
        if (animated) {
            ...
            kind = spkSemanticIsSticker(animated, &foundSticker) && foundSticker
                ? SPKDeletedMessageKindSticker : SPKDeletedMessageKindGif;
            mediaURL = spkURLStringValue(spkTryObjectSelector(animated, @"gifURL"));
            if (!mediaURL.length) mediaURL = spkGiphyMediaURL(animated);
    ...
    if (reshare && kind == SPKDeletedMessageKindUnknown) {
        ...
        if (!text.length) text = spkExtractShareTitle(reshare);
        if (!text.length) text = spkTryStringSelectors(reshare,
    ...
    if (kind == SPKDeletedMessageKindUnknown) {
        ...
        if (xmaLike) {
            ...
            if (isAudio)                                           kind = SPKDeletedMessageKindAudioShare;
            else if ([xmaContentType isEqualToString:@"xma_link"]) kind = SPKDeletedMessageKindLink;
            else                                                   kind = SPKDeletedMessageKindShare;
            ...
            NSString *(^pickStr)(id, NSArray<NSString *> *) = ^NSString *(id obj, NSArray<NSString *> *keys) {
                for (NSString *k in keys) {
                    @try {
                        ...
                        if ([v isKindOfClass:[NSString class]] && [(NSString *)v length] > 0
                            && !spkIsDescriptionFallback(v)) return v;
                    } @catch (__unused id e) {}
                }
                return nil;
            };
    ...
    if (kind == SPKDeletedMessageKindUnknown && text.length) kind = SPKDeletedMessageKindText;
...
static void spkDownloadToTempFile(NSURL *url, void (^done)(NSURL *file, NSError *err)) {
    if (!url) { done(nil, [NSError errorWithDomain:@"SPKDM" code:0 userInfo:nil]); return; }
    [[spkSharedSession() dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *resp, NSError *err) {
...
static void spkDownloadAndMuxVideo(NSString *videoURL, NSString *audioURL,
    ...
    if (!audioURL.length || ![SPKMediaFFmpeg isAvailable]) return;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:abs]) {
        if (!staged || spkPersistStagedPath(fname, messageId, ownerPk, NO, @"video/mp4")) return;
    ...
    dispatch_async(spkDownloadQueue(), ^{
        ...
        spkDownloadToTempFile(vURL, ^(NSURL *f, NSError *e) { if (!e) vFile = f; dispatch_semaphore_signal(sema); });
        dispatch_semaphore_wait(sema, DISPATCH_TIME_FOREVER);
        spkDownloadToTempFile(aURL, ^(NSURL *f, NSError *e) { if (!e) aFile = f; dispatch_semaphore_signal(sema); });
        dispatch_semaphore_wait(sema, DISPATCH_TIME_FOREVER);
        if (!vFile || !aFile) {
...
static void spkDownloadMedia(NSString *urlString, NSString *messageId,
    ...
    if (!abs.length) return;
    if ([[NSFileManager defaultManager] fileExistsAtPath:abs]) {
        if (!staged || spkPersistStagedPath(fname, messageId, ownerPk, isThumbnail, nil)) return;
        [[NSFileManager defaultManager] removeItemAtPath:abs error:nil];
    }
    ...
    dispatch_async(spkDownloadQueue(), ^{
        NSURLSessionDataTask *task = [spkSharedSession() dataTaskWithURL:url
            ...
            if (!isThumbnail && kind == SPKDeletedMessageKindVoice) detectedExt = @"m4a";
...
static void spkStageRecoverySnapshot(NSDictionary *snapshot, NSString *ownerPk) {
    ...
    if (!disappearing && kind != SPKDeletedMessageKindGif && kind != SPKDeletedMessageKindSticker) return;
    ...
    if (thumbnailURL.length) spkDownloadMedia(thumbnailURL, messageId, ownerPk, YES, kind, YES);
}
...
static id spkFindMessageInFetchedThread(id value, NSString *sid, NSInteger depth, NSMutableSet *visited) {
    ...
    if ([spkSidFromMessage(value) isEqualToString:sid]) return value;
...
NSArray<NSDictionary *> *spkDMCapturePreviewMetadataForKeys(NSArray *keys,
    ...
    for (id key in keys) {
        ...
        if (senderPk.length && [SPKDeletedMessagesStorage isSenderBlocked:senderPk ownerPK:owner]) continue;
...
static BOOL spkFinalizeSnapshot(NSDictionary *snap, NSString *sid, NSString *thread, NSString *owner) {
    ...
    if ((kind == SPKDeletedMessageKindUnknown || kind == SPKDeletedMessageKindOther)
    ...
    if (![SPKDeletedMessagesStorage saveMessage:m forOwnerPK:owner]) return NO;
    ...
    if (!m.thumbnailPath.length && m.thumbnailURL.length) spkDownloadMedia(m.thumbnailURL, sid, owner, YES, m.kind, NO);
    return YES;
}
...
static void spkFetchThreadForPendingRemoval(id applicator, NSString *sid, NSString *thread, NSString *owner) {
    ...
    @synchronized (spkPendingFetches()) {
        if ([spkPendingFetches() containsObject:fetchKey]) return;
        [spkPendingFetches() addObject:fetchKey];
    }

    void (^completion)(id) = ^(id fetchedThread) {
        @synchronized (spkPendingFetches()) { [spkPendingFetches() removeObject:fetchKey]; }
        ...
        if (!snap) return;
        dispatch_async(spkCaptureQueue(), ^{ spkFinalizeSnapshot(snap, sid, thread, owner); });
    };
...
void spkDMCaptureNoteRemoveKeys(NSArray *keys, id applicator,
    ...
    if (!spkCaptureEnabled() || !keys.count) return;
    NSString *owner  = ownerPk.length  ? [ownerPk copy]  : @"";
    NSString *thread = threadId.length ? [threadId copy] : nil;

    // Resolve the real group name from IG's cache (deduped per thread).
    if (thread.length && owner.length) spkDMCaptureResolveThreadMeta(applicator, thread, owner);
    ...
    dispatch_async(spkCaptureQueue(), ^{
        for (id key in keys) {
            NSString *sid = spkExtractKeySid(key);
            NSDictionary *snap = [SPKDeletedMessagesStorage pendingCandidateSnapshotForMessageId:sid ownerPK:owner];
            if (!snap && strongRefs[sid]) snap = spkBuildSnapshot(strongRefs[sid], owner);
            if (snap) spkFinalizeSnapshot(snap, sid, thread, owner);
        }
    });
}

void spkDMCaptureRetryPendingRemovals(id applicator, NSString *ownerPk) {
    if (!spkCaptureEnabled() || !ownerPk.length) return;
    ...
    dispatch_async(spkCaptureQueue(), ^{
        for (NSDictionary *entry in pending) {
            ...
            if (!snap) {
                ...
                if (message) snap = spkBuildSnapshot(message, owner);
            }
            if (snap) spkFinalizeSnapshot(snap, sid, thread, owner);
            else spkFetchThreadForPendingRemoval(applicator, sid, thread, owner);
        }
    });
}
...
static NSString *spkGroupCustomName(id metadata) {
    ...
    if (!name.length) name = spkStrIvar(groupMeta, "_customName");
...
static NSArray<NSString *> *spkThreadUserNames(id metadata) {
    ...
    for (id u in (NSArray *)users) {
        ...
        if (!n.length) n = spkTryStringSelectors(u, @[@"username"]);
...
static BOOL spkExtractThreadMeta(id threadObj, BOOL *outIsGroup, NSString **outTitle, NSString **outPhotoURL) {
    ...
    if (!found) isGroup = spkBoolIvar(meta, "_isGroup", &found);
    if (!found) return NO;
    NSString *title = nil;
    NSString *photo = nil;
    if (isGroup) {
        title = spkGroupCustomName(meta);
        if (!title.length) title = spkJoinThreadNames(spkThreadUserNames(meta));
...
void spkDMCaptureResolveThreadMeta(id applicator, NSString *threadId, NSString *ownerPk) {
    if (!spkCaptureEnabled() || !threadId.length || !ownerPk.length) return;
    NSString *key = [NSString stringWithFormat:@"%@:%@", ownerPk, threadId];
    @synchronized (spkResolvedThreadMetaKeys()) {
        if ([spkResolvedThreadMetaKeys() containsObject:key]) return;
    ...
    void (^clearKey)(void) = ^{
        @synchronized (spkResolvedThreadMetaKeys()) { [spkResolvedThreadMetaKeys() removeObject:key]; }
    };
    ...
    BOOL (^apply)(id) = ^BOOL(id threadObj) {
        ...
        if (!spkExtractThreadMeta(threadObj, &isGroup, &title, &photo)) return NO;
        if (isGroup) [SPKDeletedMessagesStorage backfillThreadTitle:title isGroup:YES photoURL:photo forThreadId:tid ownerPK:owner];
        return YES;
    };
...
static id spkResolveReactionTargetMessage(NSString *messageId, id applicator, NSString *threadId) {
    ...
    if (!msg) msg = spkFallbackLookupMessage(applicator, messageId, threadId);
    return msg;
}
...
NSDictionary *spkDMCaptureNoteReactionUnsend(id reaction,
                                             ...
                                             NSString *ownerPk,
                                             NSString *threadId) {
    if (!spkReactionCaptureEnabled() || !reaction) return nil;
    ...
    NSDate *now = [NSDate date];
    dispatch_async(spkCaptureQueue(), ^{
        if (pk.length && [SPKDeletedMessagesStorage isSenderBlocked:pk ownerPK:owner]) return;
