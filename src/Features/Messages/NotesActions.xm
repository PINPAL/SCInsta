...
// Resolve the IGDirectNoteModel from a tray user view-model (RyukGram: -note, or
// the view-model itself). Falls back to a shallow probe of related accessors.
...
// The note's audio track object. RyukGram resolves it via the tray view-model's
// -audioTrackWithUserMap:launcherSet: (nil user map, session launcher set); we keep
// the note's music-info accessors as fallbacks for the download coordinator.
