...
static BOOL spkThreadBlockedBySeenList(NSString *threadId) {
	if (![SPKUtils getBoolPref:@"msgs_deleted_log_respect_seen_list"]) return NO;
...
static NSMutableDictionary<NSString *, NSMutableSet<NSString *> *> *spkPreservedStore(void) {
	if (spkPreservedByPk) return spkPreservedByPk;
	...
	if ([saved isKindOfClass:NSDictionary.class]) {
		for (NSString *pk in saved) {
			...
			if (arr.count) spkPreservedByPk[pk] = [NSMutableSet setWithArray:arr];
		}
	}
...
static void spkSavePreservedIds(void) {
	...
	for (NSString *pk in spkPreservedStore()) {
		...
		while (set.count > SPK_PRESERVED_MAX) [set removeObject:set.anyObject];
		if (set.count) out[pk] = set.allObjects;
	}

	if (out.count) [NSUserDefaults.standardUserDefaults setObject:out forKey:SPK_PRESERVED_IDS_KEY];
	else [NSUserDefaults.standardUserDefaults removeObjectForKey:SPK_PRESERVED_IDS_KEY];
}
...
static void spkPruneDeleteForYouKeys(void) {
	if (!spkDeleteForYouKeys.count) return;
	NSDate *cutoff = [NSDate dateWithTimeIntervalSinceNow:-10.0];
	for (NSString *sid in spkDeleteForYouKeys.allKeys) {
		if ([spkDeleteForYouKeys[sid] compare:cutoff] == NSOrderedAscending) [spkDeleteForYouKeys removeObjectForKey:sid];
	}
}
...
static void spkCaptureMessage(id message) {
	...
	if (pk.length) spkTrackSenderPk(sid, pk);
...
static BOOL spkKeysContainDeleteForYouSid(NSArray *keys) {
	for (id key in keys) {
		NSString *sid = spkServerIdFromKey(key);
		if (sid.length && spkDeleteForYouKeys[sid]) return YES;
	}
	return NO;
}
...
static void spkRemoveDeleteForYouSids(NSArray *keys) {
	for (id key in keys) {
		NSString *sid = spkServerIdFromKey(key);
		if (sid.length) [spkDeleteForYouKeys removeObjectForKey:sid];
	}
}

static void spkTrackDeleteForYouKeys(NSArray *keys) {
	if (!spkDeleteForYouKeys) spkDeleteForYouKeys = NSMutableDictionary.dictionary;
	NSDate *now = NSDate.date;

	for (id key in keys) {
		NSString *sid = spkServerIdFromKey(key);
		if (sid.length) spkDeleteForYouKeys[sid] = now;
	}
}
...
static BOOL spkHandleReactionMutation(id mutation, NSString *messageId, NSString *ownerPk, NSString *threadId, id applicator) {
	...
	if (!reactorPk.length) reactorPk = spkStringValue(spkIvar(reaction, "_userBasedReaction_userId"));
	// Skip the owner removing their own reaction.
	if (reactorPk.length && ownerPk.length && [reactorPk isEqualToString:ownerPk]) return NO;
	if (reactorPk.length && [SPKDeletedMessagesStorage isSenderBlocked:reactorPk ownerPK:ownerPk]) return YES;
	...
	if (notifyOn) {
		...
		if (preview.count) {
			if (!spkPendingReactionPreviews) spkPendingReactionPreviews = NSMutableArray.array;
...
static void spkProcessReactionMutations(id update, NSString *ownerPk, NSString *threadId, id applicator) {
	if (!update) return;
	if (!spkReactionLogEnabled() && !spkReactionNotifyEnabled()) return;
	...
	if ([multi isKindOfClass:NSArray.class]) {
		for (id pair in (NSArray *)multi) {
			NSString *mid = nil;
			id mutation = nil;
			@try { mid = spkStringValue([pair valueForKey:@"messageId"]); } @catch (__unused id e) {}
			@try { mutation = [pair valueForKey:@"contentMutation"]; } @catch (__unused id e) {}
			if (mutation) spkHandleReactionMutation(mutation, mid, ownerPk, threadId, applicator);
		}
	}
}
...
static BOOL spkProcessMessageUpdate(id update, NSString *ownerPk, NSString *threadId, id applicator, NSMutableSet<NSString *> *preserved, NSMutableSet<NSString *> *detected, NSMutableArray<NSDictionary *> *previews, BOOL loggingAllowed) {
	...
	if (loggingAllowed) spkProcessReactionMutations(update, ownerPk, threadId, applicator);
	...
	for (id key in keys) {
		...
		if (spkIsReactionOrActionLog(sid)) continue;
		NSString *senderPk = spkSenderMap()[sid];
		if (senderPk.length && [SPKDeletedMessagesStorage isSenderBlocked:senderPk ownerPK:ownerPk]) continue;
		if (senderPk.length) spkTrackSentByOwner(sid, [senderPk isEqualToString:ownerPk]);
	...
	if (logOn) spkDMCaptureNoteRemoveKeys(unsendKeys, applicator, ownerPk, threadId);
	if (keepOn && didPreserve) spkSetIvar(update, "_removeMessages_messageKeys", nil);
...
static NSSet<NSString *> *spkProcessCacheUpdate(id cacheUpdate, NSString *ownerPk, id applicator, NSMutableSet<NSString *> *detected, NSMutableArray<NSDictionary *> *previews) {
	...
	for (id tu in threadUpdates) {
		...
		if (msgUpdate) spkProcessMessageUpdate(msgUpdate, ownerPk, threadId, applicator, preserved, detected, previews, loggingAllowed);
	}
...
static NSString *spkNotificationKindPhrase(SPKDeletedMessageKind kind) {
	switch (kind) {
		case SPKDeletedMessageKindPhoto: return @"photo";
		case SPKDeletedMessageKindVideo: return @"video";
		case SPKDeletedMessageKindVoice: return @"voice message";
		case SPKDeletedMessageKindGif: return @"GIF";
		case SPKDeletedMessageKindSticker: return @"sticker";
		case SPKDeletedMessageKindShare: return @"share";
		case SPKDeletedMessageKindLink: return @"link";
		case SPKDeletedMessageKindAudioShare: return @"music share";
...
static BOOL spkShouldShowUnsentToast(NSArray<NSString *> *dedupeKeys) {
	...
	if (!spkUnsentToastDedupe) spkUnsentToastDedupe = NSMutableDictionary.dictionary;
	...
	for (NSString *dedupeKey in dedupeKeys) {
		if (dedupeKey.length) spkUnsentToastDedupe[dedupeKey] = now;
	}
	return YES;
}
...
static void spkShowUnsentToast(NSDictionary *preview, NSString *fallbackSender, NSString *fallbackSenderPk, NSString *fallbackThreadId, NSString *ownerAccount, NSString *fallbackSid) {
	...
	if (!spkShouldShowUnsentToast(dedupeKeys)) return;
...
static void spkShowUnsentReactionToast(NSDictionary *preview, NSString *ownerAccount) {
	...
	if (!spkShouldShowUnsentToast(@[dedupeKey])) return;
...
static void spkHandleApplyUpdates(id self, id updates, void (^invokeOriginal)(void)) {
	...
	if (logOn && ownerPk.length) spkDMCaptureRetryPendingRemovals(self, ownerPk);
	NSMutableSet *preserved = NSMutableSet.set;
	...
	if (preserved.count) spkSavePreservedIds();
	...
	if (!toastSenderName.length && toastSenderPk.length) {
		...
		if (toastSenderName.length) spkTrackSenderName(toastSid, toastSenderName);
	}

	dispatch_async(dispatch_get_main_queue(), ^{
		if (foreground) spkRefreshVisibleCellIndicators();
		if (toastOn) {
			if (toastPreviews.count) {
				for (NSDictionary *preview in toastPreviews) spkShowUnsentToast(preview, toastSenderName, toastSenderPk, nil, ownerName, toastSid);
		...
		if (reactionPreviews.count) {
			for (NSDictionary *preview in reactionPreviews) spkShowUnsentReactionToast(preview, ownerName);
		}
	});
}
...
static void new_removeMutationExecute(id self, SEL _cmd, id handler, id pkg) {
	...
	if ([keys isKindOfClass:NSArray.class]) {
		if (reason == 2) {
			...
			for (id key in keys) {
				NSString *sid = spkServerIdFromKey(key);
				if (sid.length) [spkPendingLocalSet() addObject:sid];
			}
		}
	}
...
static void new_configureCell(id self, SEL _cmd, id vm, id ringSpec, id launcherSet) {
	...
	if (sid.length) {
		...
		if (pk.length) {
			...
			NSString *currentPk = spkCurrentUserPk();
			if (currentPk.length) spkTrackSentByOwner(sid, [pk isEqualToString:currentPk]);
		}
	}
...
static void new_addAccessory(id self, SEL _cmd, id view) {
	...
	if (!spkIndicatorEnabled() || !spkCellIsPreserved(self) || ![view isKindOfClass:UIView.class]) return;
...
static id new_actionLogInit(id self, SEL _cmd, id message, id title, id attrs, id parts, id type, BOOL collapsible, BOOL hidden, id genAI) {
	...
	@try {
		...
		if ([result respondsToSelector:sel]) {
			...
			if (sid.length) spkTrackContentClass(sid, @"IGDirectThreadActionLog");
		}
	} @catch (__unused id e) {}
...
%ctor {
	...
	if (!spkHook(cacheCls, NSSelectorFromString(@"_applyThreadUpdates:completion:userAccess:"), (IMP)new_applyUpdatesUserAccess, (IMP *)&orig_applyUpdatesUserAccess)
		&& !spkHook(cacheCls, NSSelectorFromString(@"_applyThreadUpdates:completion:"), (IMP)new_applyUpdatesCompletion, (IMP *)&orig_applyUpdatesCompletion)) {
