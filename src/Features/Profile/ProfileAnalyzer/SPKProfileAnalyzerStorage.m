...
static id spkStripNull(id obj) {
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        for (id k in obj) {
            id v = obj[k];
            if (v && ![v isKindOfClass:[NSNull class]]) out[k] = spkStripNull(v);
        }
        return out;
    }
    if ([obj isKindOfClass:[NSArray class]]) {
        NSMutableArray *out = [NSMutableArray arrayWithCapacity:[obj count]];
        for (id v in obj) if (v && ![v isKindOfClass:[NSNull class]]) [out addObject:spkStripNull(v)];
        return out;
    }
    return obj;
}
...
+ (BOOL)replaceStorageWithDirectoryAtPath:(NSString *)sourcePath error:(NSError **)error {
    ...
    if (copied) spkPostDataChanged(nil);
    return copied;
}
...
+ (NSInteger)mergeFromStorageDirectory:(NSString *)sourcePath
    ...
    for (NSString *safePK in pks) {
        if (ownerFilterPK.length > 0 && ![safePK isEqualToString:spkSafePK(ownerFilterPK)]) continue;
        ...
        if (![fm fileExistsAtPath:spkPath(safePK, @"current")]) {
            for (NSString *slot in @[@"current", @"previous", @"baseline", @"header"]) {
                ...
                if ([fm fileExistsAtPath:src]) [fm copyItemAtPath:src toPath:spkPath(safePK, slot) error:nil];
            }
        }
        ...
        dispatch_sync(spkVisitQueue(), ^{
            ...
            for (id entry in srcList) {
                ...
                if (vpk.length == 0 || spkVisitIndexForPK(liveList, vpk) != NSNotFound) continue;
