...
- (NSError *)errorWithCode:(SPKProfileAnalyzerError)code message:(NSString *)msg {
    return [NSError errorWithDomain:@"SPKProfileAnalyzer" code:code
...
- (void)runForSelfWithHeaderInfo:(SPKPAHeaderInfo)headerInfo
                      ...
                      completion:(SPKPACompletion)completion {
    if (self.isRunning) {
        if (completion) completion(nil, [self errorWithCode:SPKProfileAnalyzerErrorAlreadyRunning
...
- (void)fetchFollowersForPK:(NSString *)pk
    ...
    [self pagePath:[NSString stringWithFormat:@"friendships/%@/followers/", pk]
        ...
        if (error || self.cancelled) {
            [self finishWithSnapshot:nil error:error ?: [self errorWithCode:SPKProfileAnalyzerErrorCancelled message:@"Cancelled"]
                          completion:completion];
            return;
        }
...
- (void)pagePath:(NSString *)basePath
    ...
    __weak typeof(self) weakSelf = self;
    [SPKInstagramAPI sendRequestWithMethod:@"GET" path:path body:nil completion:^(NSDictionary *resp, NSError *error) {
        typeof(self) strongSelf = weakSelf;
        if (!strongSelf) return;
