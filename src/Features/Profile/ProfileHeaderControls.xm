...
static void SPKApplyProfileHeaderControls(UIViewController *vc) {
    ...
    if (!hideThreads && !hideNotes && !sSPKProfileControlsEverApplied) return;
    if (hideThreads || hideNotes) sSPKProfileControlsEverApplied = YES;

    SPKApplyProfileHeaderControlsInTree(vc.view, hideThreads, hideNotes, 0);
    UIView *navBar = vc.navigationController.navigationBar;
    if (navBar) SPKApplyProfileHeaderControlsInTree(navBar, hideThreads, hideNotes, 0);
}
